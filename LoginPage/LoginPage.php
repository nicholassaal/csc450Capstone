...
function loginFunction()
{
    ...
    if (isset($_POST['userName']) && isset($_POST['password'])) {
        ...
        if (empty($userName)) { //If username is not filled in
            ...
            //if the login query returns a row in the database userLoginInfo table
...
<body>
    <header>
        <h1>CSP Student Review Login Page</h1>
        <!--Nav Bar, used an Unorder lists-->
    </header>

    <div class="loginContainer">
        <!--Image and Login info-->
        <img src="image/CSPpicture.jpg" alt="Picture of CSP">

        <section id="userInformation">
            <!--Container for login user informatoin -->
            <h1 id="loginHeader">Login</h1>
            <p>Login using your college user name and password.</p>

            <form method="POST">
                <label for="userName">Username</label>
                <!-- Creating a lable for input type of "text" then giving a name and
                                                                id to match the lable name-->
                <input type="text" name="userName" id="userName">

                <label for="password">Password</label> <!-- password input type and changed name and id to password-->
